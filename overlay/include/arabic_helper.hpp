...
private:
    ...
    static bool isArabicLetter(char32_t c) {
        return (c >= 0x0621 && c <= 0x064A) || (c >= 0xFE70 && c <= 0xFEFF);
    }
...
public:
    static std::string process(const std::string& input) {
        ...
        std::u32string line = utf8_to_utf32(input);
        bool hasArabic = false;
        for (char32_t c : line) {
            if (isArabicLetter(c)) { hasArabic = true; break; }
        }
        ...
        for (size_t i = 0; i < line.size(); ++i) {
            ...
            if (current == 0x0644 && i + 1 < line.size()) {
                char32_t next = line[i+1];
                char32_t ligature = 0;
                ...
                if (ligature != 0) {
                    bool prev_connects = (i > 0) && connectsToNext(line[i-1]);
            ...
            bool prev_connects = (i > 0) && connectsToNext(line[i-1]);
            bool next_connects = (i + 1 < line.size()) && isArabicLetter(line[i+1]);
